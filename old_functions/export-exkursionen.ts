...
export const onRequestGet: PagesFunction<Env> = async (context) => {
    ...
    let result: D1Result = await context.env.DB.prepare("SELECT id, vorname, nachname, email, exkursion1, exkursion2, exkursion3, datenschutz FROM exkursionen")
        .all<string>();
    if (!result.success) {
        console.log(result)
        return new Response("Konnte die Anmeldungen zu Exkursionen nicht abrufen", { status: 500 });
    }
    ...
    return new Response(body, {
        headers: {
            ...
            "Content-Disposition": 'attachment; filename="exkursionen_anmeldungen.csv"'
        }
    })
};